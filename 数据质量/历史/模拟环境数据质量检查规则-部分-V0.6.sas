...
datepart(ddateclosed) as ddateclosed label = "到期日期" format = yymmdd10. informat = yymmdd10.,
  ...
  from nfcs.sino_loan(where = (dbillingdate ^= drecentpaydate and iaccountstat not in (1,2) and dgetdate >= mdy(7,1,2013))) as t
;
quit;
...
/*Rule 5*/
/*9、T+1开户时，"结算应还款日期"和"最近一次实际还款日期"应该等于"开户日期" （非T+1不适用）（T+1开户月--若不需还款和没发生还款，开户日上报一次）*/
/*错误*/
/*需要更新*/
proc sql;
...
a.drecentpaydate label = '最近一次实际还款日期' format = DATETIME20. informat = DATETIME20.,
a.ischeduledamount label = "本月应还款金额",
a.iactualpayamount label = "本月实际还款金额",
a.sPaystat24month label = '二十四月还款状态'
...
a.iaccountstat   label = "账户状态",
a.sPaystat24month label = '二十四月还款状态'
  from nfcs.sino_loan(where = (substr(sPaystat24month,1,23) ^= '///////////////////////' and sTermsfreq in ('04' '05' '06') and iaccountstat ^= 3 and datepart(dbillingdate) ^= intnx('month',datepart(dbillingdate),0,'end') &timefilter.)) as a
order by sorgcode,saccount,dbillingdate
;
quit;
...
/*Rule 8*/
/*14、"本月实际还款金额"大于"本月应还款金额"，需确认是否符合实际情况*/
/*怀疑*/
proc sql;
...
datepart(ddateclosed) as ddateclosed label = "到期日期" format = yymmdd10. informat = yymmdd10.,
       ...
       a.iactualpayamount label = "本月实际还款金额",
       a.sPaystat24month  label = '二十四月还款状态'
  from nfcs.sino_loan(where = (iaccountstat = 3 and iactualpayamount = 0 &timefilter.)) as a
    order by sorgcode,saccount,dbillingdate
;
quit;

/*Rule 10*/
/*16、在"本月应还款金额"或"本月实际还款金额"不为0的情况下，24月还款状态不应为星号*/
/*错误*/
proc sql;
...
a.iactualpayamount label = '实际还款金额',
a.sPaystat24month label = '二十四月还款状态'
  from nfcs.sino_loan(where = (ischeduledamount ^= 0 or iactualpayamount ^= 0 and substr(sPaystat24month,24, 1) = '*' &timefilter.)) as a
    order by sorgcode,saccount,dbillingdate
;
quit;
...
a.iactualpayamount label = '实际还款金额',
a.sPaystat24month label = '二十四月还款状态'
  from nfcs.sino_loan(where = (sPaystat24month ^= '///////////////////////*' and sTermsfreq = '03' and ischeduledamount=0)) as a
    order by sorgcode,saccount,dbillingdate
;
quit;
...
a.iactualpayamount label = '实际还款金额',
a.sPaystat24month label = '二十四月还款状态'
  from nfcs.sino_loan(where = (drecentpaydate ^= dbillingdate and iaccountstat = 3)) as a
   order by sorgcode,saccount,dbillingdate
;
quit;
...
from nfcs.sino_loan(where = (sTermsfreq in ('03', '04', '05','06') and input(smonthduration,4.) < itermspastdue &timefilter.))
order by sorgcode,saccount,dbillingdate
;
quit;
...
datepart(ddateclosed) as ddateclosed label = "到期日期" format = yymmdd10. informat = yymmdd10.,
  ...
  from nfcs.sino_loan( where = (substr(sPaystat24month,23,1) in ('*','#','/','N') and  ischeduledamount ^= iactualpayamount  and  ddateclosed = dbillingdate )) as a
 order by sorgcode,saccount,dbillingdate
 ;
 quit;
...
/*		,(case when t2.dmonth is not null then 1 else 0 end) as status label='入库状态'*/
		...
		order by sorgcode,saccount,dmonth desc
;
quit;
/*展示逻辑为：结束日期早于当前，至少应报送结束当月的账期；结束日期晚于当前，至少应报送当前月份的账期*/
data rule_25;
	set rule_25;
	if cmonth <= %sysfunc(intnx('month',today(),-2,0)) and cmonth > dmonth then delete;
	if cmonth > %sysfunc(intnx('month',today(),-2,0)) and dmonth < %sysfunc(intnx('month',today(),-2,0)) then delete;
run;
...
/*怀疑*/
proc sort data = nfcs.sino_loan(keep = iloanid sorgcode saccount dbillingdate SMONTHDURATION icreditlimit ITREATYPAYAMOUNT STREATYPAYDUE sPaystat24month WHERE= (SUBSTR(sorgcode,1,1)='Q' AND sorgcode not in ('Q10152900H0000','Q10152900H0001'))) out = rule_28_t nodupkey;
by iloanid descending dbillingdate;
run;
/*标记非等额还款的业务*/
/*需要使用正则表达式识别出开户月及随后无需还款的月份*/
/* where= (PRXMATCH('[\\#\*]',sPaystat24month) = 1) */
data rule_28_t1;
	set rule_28_t(keep = iloanid ITREATYPAYAMOUNT );
	ITREATYPAYAMOUNT_NUM = INPUT(ITREATYPAYAMOUNT,BEST12.);
	if iloanid = lag(iloanid) and ITREATYPAYAMOUNT_NUM ^= lag(ITREATYPAYAMOUNT_NUM) and ITREATYPAYAMOUNT_NUM ^= 0 then denger_label = 1;
run;

proc sql;
create table rule_28_t2 as select
		iloanid
	from rule_28_t1
		group by iloanid
		having sum(denger_label) > 0
;
quit;
...
proc sql;
	create table rule_28 as select
	*
	from rule_28_t4
	where iloanid not in (select * from rule_28_t2)
;
quit;
...
stop;
%do i = 1 %to &socnumber.;
	rc =system("'C:/Program Files/WinRAR/winrar.exe' a -ad -icbk -ep -y &outfile.%sysfunc(trim(&&shortname&i.))_&currmonth..rar D:\逻辑校验结果\数据质量检查系统输出结果说明-V1.1.docx");
